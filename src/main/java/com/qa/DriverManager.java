...
public class DriverManager{
    ...
    public static void initializeDriver (String envID, String deviceID) throws Exception {
        ...
        caps.setCapability("device", /*deviceObj.getString("device")*/"Google Pixel 3");
        caps.setCapability("os_version", /*deviceObj.getString("os_version")*/"10.0");
        caps.setCapability("project", "Sauce-Lab-App-Upload");
        caps.setCapability("build", "NHK");
        caps.setCapability("name", "Sample Test Run");

        if(envID.equals("local")) {
            System.out.println("Execution will happen in Local");
        }
        else if(envID.equals("remote")) {
            switch (deviceID) {
                case "1":
                    caps.setCapability("app", /*deviceObj.getString("app_url")*/"storage:1c0dd137-09ab-4534-8263-5053d72b2d98");
                    driver = new AndroidDriver(url, caps);
                    break;
                case "2":
                    caps.setCapability("app", /*deviceObj.getString("app_url")*/"storage:81c3825c-d215-424a-bb8f-a648f210d958");
